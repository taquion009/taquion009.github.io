...
const meshNeon = () => {
    ...
    class Hexagon {
        ...
        constructor(x: number, y: number, color: string) {
            ...
            this.y = y
            this.color = color
            this.middleX = (this.x-(sizeH - gap)-(sizeH - gap)*Math.sin(Math.PI/6))+(widthH/2)
    ...
    document.querySelector<HTMLElement>(".s1").addEventListener('mousemove', (e) => {
        const x = e.clientX
        const y = e.clientY
        const hexagon = hexagons.find(hex => 
            x < hex.middleX + (widthH/2) && x > hex.middleX - (widthH/2) &&
            y < hex.middleY + (heightH/2) && y > hex.middleY - (heightH/2)
            )
        if (hexagon) {
            hexagon.drawM = true
        }
    ...
    document.querySelector<HTMLElement>(".s1").addEventListener('touchmove', (event) => {
