...
const screenFull = () => {
  ...
  let qty = 1,
    ...
    stickCurrent: null | HTMLElement =
      document.querySelector(`.stick[data-to="${qty}"]`) || null,
  ...
  const changeActiveStick = (qty2: number) => {
    ...
    let stick: HTMLElement = document.querySelector(
      `.stick[data-to="${qty2}"]`
    );
    if (!stick) return;
    if (stick) {
    ...
    lineActive.style.transform = `translateX(${
      stick.offsetLeft + stick.offsetWidth / 2 / 2
    }px)`;
    lineActive.style.transition = "transform 0.5s";
    lineActive.style.width = `${stick.offsetWidth / 2}px`;
  };

  document.querySelectorAll(".move-screen").forEach((stick) => {
    stick.addEventListener("click", (e) => {
      ...
      if (!mobile) {
        const to =
          Number(e.target.parentElement.dataset.to) ||
          Number(e.target.dataset.to);
        ...
        if (startFlag) {
          ...
          for (let i = Number(to) + 1; i <= sectionsQty; i++) {
            ...
            document.querySelector<HTMLElement>(`section.s${i}`).style.display =
              "none";
  ...
  window.addEventListener("resize", () => {
    ...
    if (mobile) {
      for (let i = 1; i <= sectionsQty; i++) {
        document.querySelector<HTMLElement>(`section.s${i}`).style.transform =
          "translateY(0)";
      }
    } else {
      document.querySelector<HTMLElement>(`section.s1`).style.transform =
        "translateY(0)";
